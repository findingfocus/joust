...
---[[TEMPORARY SAFETY SPRITE SWITCHING
	...
	if love.keyboard.isDown('o') or love.keyboard.isDown('p') or love.keyboard.isDown('i') then
...
---[[INPUT HANDLING
			...
			if love.keyboard.isDown('o') and not love.keyboard.isDown('p') then
				self.rightPriority = false
				lastInput = {}
				table.insert(lastInput, "left")

			elseif love.keyboard.isDown('p') and not love.keyboard.isDown('o') then
				self.rightPriority = true
				lastInput = {}
				table.insert(lastInput, "right")

			elseif love.keyboard.isDown('p') and love.keyboard.isDown('o') and not self.lastInputLocked then
			...
			if love.keyboard.wasReleased('o') or love.keyboard.wasReleased('p') then
				self.lastInputLocked = false
			end
			...
			if self.grounded then
				...
				if love.keyboard.isDown('o') and lastInput[1] == "left" and self.dx >= SKIDTHRESHOLD and not self.skid then
				...
				if love.keyboard.isDown('p') and lastInput[1] == "right" and self.dx <= -SKIDTHRESHOLD and not self.skid then
					sounds['leftStep']:stop()
					sounds['rightStep']:stop()
					sounds['skid']:play()
					self.skid = true
				...
				if self.facingRight then
					...
					if love.keyboard.isDown('p') and lastInput[1] == "right" and not self.skid and not self.justTurned then
						self.dx = math.max(.12, (math.min(self.dx + SPEEDRAMP, MAXSPEED)))
					end

					if self.dx == 0 then
						--TURNS LEFT WHEN STOPPED
						if love.keyboard.isDown('o') and lastInput[1] == "left" and not self.justStopped then
							self.facingRight = false
					...
					elseif (self.dx > 0 and self.dx < SKIDTHRESHOLD) then
						...
						if love.keyboard.isDown('o') and lastInput[1] == "left" then
							self.dx = 0
					...
					elseif self.dx >= SKIDTHRESHOLD then
						if love.keyboard.isDown('o') and love.keyboard.wasReleased('p') then
							self.skid = true
							sounds['skid']:play()
						end

						if love.keyboard.wasPressed('o') and not self.skid then
							self.skid = true
							sounds['skid']:play()
							sounds['leftStep']:stop()
							sounds['rightStep']:stop()
				...
				elseif not self.facingRight then
					...
					if love.keyboard.isDown('o') and lastInput[1] == "left" and not self.skid and not self.justTurned then
							self.dx = math.min(-.12, (math.max(self.dx - SPEEDRAMP, -MAXSPEED)))
					end

					if self.dx == 0 then
						--TURNS RIGHT WHEN STOPPED
						if love.keyboard.isDown('p') and lastInput[1] == "right" and not self.justStopped then
							self.facingRight = true
							self.justTurned = true
						end
					...
					elseif (self.dx < 0 and self.dx > -SKIDTHRESHOLD) then
						...
						if love.keyboard.isDown('p') and lastInput[1] == "right" then
							self.dx = 0
							self.justStopped = true
						end
					--SKID FLAG
					elseif self.dx < -SKIDTHRESHOLD then
						if love.keyboard.isDown('p') and love.keyboard.wasReleased('o') then
								self.skid = true
								sounds['leftStep']:stop()
								sounds['rightStep']:stop()
								sounds['skid']:play()
						...
						if love.keyboard.wasPressed('p') and not self.skid then
							self.skid = true
							sounds['leftStep']:stop()
							sounds['rightStep']:stop()
							sounds['skid']:play()
			...
			elseif not self.grounded then
				...
				if love.keyboard.isDown('o') and lastInput[1] == 'left' and love.keyboard.wasPressed('i') then
					self.dx = math.max(self.dx - FLAPAMOUNT, -MAXSPEED)
				end

				if love.keyboard.isDown('p') and lastInput[1] == 'right' and love.keyboard.wasPressed('i') then
...
---[[OSTRICH1 ANIMATION CYCLE
			...
			--PLAYER AERIAL ANIMATION
				if not self.grounded then
					...
					if (love.keyboard.wasPressed('a') or love.keyboard.wasPressed('i')) then
						self.jumpTimer = 0
						self.flapped = true
					end
